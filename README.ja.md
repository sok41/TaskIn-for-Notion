...
- 「連続して登録する」トグルで、登録後もウィンドウを閉じずに続けて複数タスクを入力可能
- システムトレイに常駐し、バックグラウンドで待機
- Windows起動時の自動起動、表示言語（日本語/English）の切り替えに対応
- 設定画面の「アップデートを確認」ボタンで、GitHub上の最新版をチェック
...
このアプリはまだGitHub上にリリースを公開していないため、現時点で「アップデートを確認」を押すと「まだリリースが公開されていません」と表示されます。[GitHub Releases](https://github.com/sok41/TaskIn-for-Notion/releases)に新しいバージョンを公開すると、設定画面のボタンからそれを検知し、ダウンロードページへのリンクが表示されるようになります。
